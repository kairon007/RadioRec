 ...
 * operations. It provides synchronous (blocking) and asynchronous (non-blocking) methods for many common in-app billing operations, as well as automatic
 * signature verification.
 * 
 * After instantiating, you must perform setup in order to start using the object. To perform setup, call the {@link #startSetup} method and provide a listener;
 * that listener will be notified when setup is complete, after which (and not before) you may call other methods.
 * 
 * After setup is complete, you will typically want to request an inventory of owned items and subscriptions. See {@link #queryInventory},
 * {@link #queryInventoryAsync} and related methods.
 * 
 * When you are done with this object, don't forget to call {@link #dispose} to ensure proper cleanup. This object holds a binding to the in-app billing
 * service, which will leak unless you dispose of it correctly. If you created the object on an Activity's onCreate method, then the recommended place to
 * dispose of it is the Activity's onDestroy method.
 * 
 * A note about threading: When using this object from a background thread, you may call the blocking versions of methods; when using from a UI thread, call
 * only the asynchronous versions and handle the results via callbacks. Also, notice that you can only call one asynchronous operation at a time; attempting to
 * start a second asynchronous operation while the first one has not yet completed will result in an exception being thrown.
 * 
...
public class IabHelper {
	...
	/**
	 * Creates an instance. After creation, it will not yet be ready to use. You must perform setup by calling {@link #startSetup} and wait for setup to
	 * complete. This constructor does not block and is safe to call from a UI thread.
	 * 
	 * @param ctx
	 *            Your application or Activity context. Needed to bind to the in-app billing service.
	 * @param base64PublicKey
	 *            Your application's public key, encoded in base64. This is used for verification of purchase signatures. You can find your app's base64-encoded
	 *            public key in your application's page on Google Play Developer Console. Note that this is NOT your "developer public key".
	 */
	...
	/**
	 * Callback for setup process. This listener's {@link #onIabSetupFinished} method is called when the setup process is complete.
	 */
	...
	/**
	 * Starts the setup process. This will start up the setup process asynchronously. You will be notified through the listener when the setup process is
	 * complete. This method is safe to call from a UI thread.
	 * 
	 * @param listener
	...
	/**
	 * Dispose of object, releasing resources. It's very important to call this method when you are done with this object. It will release any resources used by
	 * it such as service connections. Naturally, once the object is disposed of, it can't be used again.
	 */
	public void dispose() {
		logDebug("Disposing.");
		mSetupDone = false;
	...
	public interface OnIabPurchaseFinishedListener {
		/**
		 * Called to notify that an in-app purchase finished. If the purchase was successful, then the sku parameter specifies which item was purchased. If the
		 * purchase failed, the sku and extraData parameters may or may not be null, depending on how far the purchase process went.
		 * 
		 * @param result
	...
	/**
	 * Handles an activity result that's part of the purchase flow in in-app billing. If you are calling {@link #launchPurchaseFlow}, then you must call this
	 * method from your Activity's {@link android.app.Activity@onActivityResult} method. This method MUST be called from the UI thread of the Activity.
	 * 
	 * @param requestCode
	 ...
	 * @return Returns true if the result was related to a purchase flow and was handled; false if the result was not related to a purchase, in which case you
	...
	/**
	 * Queries the inventory. This will query all owned items from the server, as well as information on additional skus, if specified. This method may block or
	 * take long to execute. Do not call from a UI thread. For that, use the non-blocking version {@link #refreshInventoryAsync}.
	 * 
	 * @param querySkuDetails
	 *            if true, SKU details (price, description, etc) will be queried as well as purchase information.
	 * @param moreItemSkus
	 *            additional PRODUCT skus to query information on, regardless of ownership. Ignored if null or if querySkuDetails is false.
	 * @param moreSubsSkus
	 *            additional SUBSCRIPTIONS skus to query information on, regardless of ownership. Ignored if null or if querySkuDetails is false.
	 * @throws IabException
	...
	/**
	 * Asynchronous wrapper for inventory query. This will perform an inventory query as described in {@link #queryInventory}, but will do so asynchronously and
	 * call back the specified listener upon completion. This method is safe to call from a UI thread.
	 * 
	 * @param querySkuDetails
	...
	/**
	 * Consumes a given in-app product. Consuming can only be done on an item that's owned, and as a result of consumption, the user will no longer own it. This
	 * method may block or take long to return. Do not call from the UI thread. For that, see {@link #consumeAsync}.
	 * 
	 * @param itemInfo
	...
	/**
	 * Asynchronous wrapper to item consumption. Works like {@link #consume}, but performs the consumption in the background and notifies completion through the
	 * provided listener. This method is safe to call from a UI thread.
	 * 
	 * @param purchase
